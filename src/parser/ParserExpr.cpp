...
ptr<Expr> Parser::parse_primary() {
    ...
    if (!(restrictions & OnlyTypeExpr)) {
        ...
        if (m_nextToken.type == TokenType::kw_atomicLoad) {
            return parse_atomic_load_expr();
        }
        if (m_nextToken.type == TokenType::kw_atomicStore) {
            return parse_atomic_store_expr();
        }
        if (m_nextToken.type == TokenType::kw_atomicCmpExW || m_nextToken.type == TokenType::kw_atomicCmpExS) {
            return parse_atomic_cmpex_expr();
        }
        if (m_nextToken.type == TokenType::kw_atomicRmw) {
            return parse_atomic_rmw_expr();
        }
    }
    if (restrictions & OnlyTypeExpr) {
...
ptr<AtomicLoadExpr> Parser::parse_atomic_load_expr() {
    debug_func("");
    matchOrReturn(TokenType::kw_atomicLoad, "expected @atomicLoad");
    auto location = m_nextToken.loc;
    eat_next_token();
    ...
    varOrReturn(ptr_expr, parse_expr());

    matchOrReturn(TokenType::par_r, "expected ')'");
    eat_next_token();

    return makePtr<AtomicLoadExpr>(location, std::move(ptr_expr));
}

ptr<AtomicStoreExpr> Parser::parse_atomic_store_expr() {
    debug_func("");
    matchOrReturn(TokenType::kw_atomicStore, "expected @atomicStore");
    auto location = m_nextToken.loc;
    eat_next_token();
    ...
    varOrReturn(val_expr, parse_expr());

    matchOrReturn(TokenType::par_r, "expected ')'");
    eat_next_token();

    return makePtr<AtomicStoreExpr>(location, std::move(ptr_expr), std::move(val_expr));
}

ptr<AtomicCmpExExpr> Parser::parse_atomic_cmpex_expr() {
    debug_func("");
    bool isWeak = m_nextToken.type == TokenType::kw_atomicCmpExW;
    auto location = m_nextToken.loc;
    eat_next_token();
    ...
    varOrReturn(expected, parse_expr());

    matchOrReturn(TokenType::comma, "expected ','");
    eat_next_token();

    varOrReturn(replacement, parse_expr());

    matchOrReturn(TokenType::par_r, "expected ')'");
    eat_next_token();

    return makePtr<AtomicCmpExExpr>(location, std::move(ptr_expr), std::move(expected), std::move(replacement), isWeak);
}

ptr<AtomicRmwExpr> Parser::parse_atomic_rmw_expr() {
    debug_func("");
    auto location = m_nextToken.loc;
    eat_next_token();
    ...
    auto op = m_nextToken.type;
    // Validate that it's a supported operator
    if (op != TokenType::op_plus && op != TokenType::op_minus && op != TokenType::amp && op != TokenType::pipe &&
        op != TokenType::caret && op != TokenType::op_assign && op != TokenType::op_less && op != TokenType::op_more) {
        return report(m_nextToken.loc, "expected atomic operator (+, -, &, |, ^, <, >, =)");
    }
    eat_next_token();
    ...
    return makePtr<AtomicRmwExpr>(location, std::move(ptr_expr), op, std::move(val_expr));
}
